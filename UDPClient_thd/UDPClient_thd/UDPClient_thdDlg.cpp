...
BEGIN_MESSAGE_MAP(CUDPClient_thdDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_SEND, &CUDPClient_thdDlg::OnClickedSend)
	ON_BN_CLICKED(IDC_CLOSE, &CUDPClient_thdDlg::OnClickedClose)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_FILE, &CUDPClient_thdDlg::OnBnClickedFile)
END_MESSAGE_MAP()
...
UINT TXThread(LPVOID arg)
{
	...
	pDlg->m_ipAddr.GetWindowText(addr);
	...
	while (pArg->Thread_run)
	{
		...
		if (ack_pos != NULL)
		{
			...
			{
				pDlg->m_pDataSocket->SendTo((TCHAR*)&ack_pkt, sizeof(struct Packet), 8000, addr);
			...
			else if (ack_pkt.role == RNR)
			{
				...
				if (pos != NULL &&pTempList->GetCount() < MAX_WINDOW_SIZE)
				{
					...
					{
						pDlg->m_pDataSocket->SendTo((TCHAR*)&tx_pkt, sizeof(struct Packet), 8000, addr);
						time_cs.Lock();
...
UINT RXThread(LPVOID arg)
{
	...
	while (pArg->Thread_run)
	{
		POSITION rpos = pRxList->GetHeadPosition();
		POSITION current_pos;
		...
		while (rpos != NULL)
		{
			...
			rx_cs.Lock();
			Packet rx_pkt = pRxList->GetNext(rpos);
			rx_cs.Unlock();
...
UINT TimerThread(LPVOID arg)
{
	...
	while (true)
	{
		...
		{
			pDlg->SetTimer(pArg->rr_num, 300000000000000000, NULL);
			time_cs.Lock();
			pArg->Thread_run = 3;
			time_cs.Unlock();
		}
...
BOOL CUDPClient_thdDlg::OnInitDialog()
{
	...
	m_pDataSocket->Create(9097, SOCK_DGRAM);
...
void CUDPClient_thdDlg::OnClickedSend()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if (m_pDataSocket == NULL)
	{
		MessageBox(_T("소켓이 생성되지않았습니다."), _T("알림"), MB_ICONERROR);
	}
	else
	{
		CString tx_message;
...
void CUDPClient_thdDlg::OnClickedClose()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if (m_pDataSocket == NULL)
	{
		MessageBox(_T("클라이언트 이미 종료"), _T("알림"), MB_ICONERROR);
	}
	else
	{
		m_pDataSocket->Close();
...
void CUDPClient_thdDlg::ProcessReceive(CDataSocket * pSocket, int nErrorCode)
{
	...
	if (rx_pkt->role == MESSAGE || rx_pkt->role == F_O || rx_pkt->role == F_N)
	{
		...
		else OutputDebugString(_T("재전송 무시합니다.\r\n"));
	}
...
void CUDPClient_thdDlg::OnTimer(UINT_PTR nIDEvent)
{
...
void CUDPClient_thdDlg::OnBnClickedFile()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
